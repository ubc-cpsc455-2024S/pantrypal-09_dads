...
- **Deep Parekh**
  - I worked on the backend, frontend and the deployment of the app. For the backend, I designed a comprehensive test suite, refactored it and added new functions at multiple points for the server routes and MongoDB connection while for the frontend, I set up the main redux functionality, made some UI improvements for recipes and ingredients while also making the initial data schemas. I also created the GitHub actions file for CI/CD and ensured the app hosted on Render properly.
- **David Mwita**
